...
def create_task(sender, instance: WbTaskModel, created, **kwargs):
    ...
    if created:
        ...
        try:
            ...
            return
        generate_supply_documents(instance, supply)

    if instance.task_state in (Status.GET_ORDERS_STICKERS.value, Status.SEND_SUPPLY_TO_DELIVER.value):
        supply = WbSupplyModel.objects.get(task=instance)
        try:
            wb_order_service.get_supply_sticker(supply=supply)
        except:
...
def generate_supply_documents(instance: WbTaskModel, supply: WbSupplyModel):
